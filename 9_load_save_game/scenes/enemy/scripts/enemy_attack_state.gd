...
var ATTACK_VELOCITY = 50.0
var ATTACK_ACCELERATION = 10.0
...
func Update(_delta:float):
	character_body.velocity.x = move_toward(character_body.velocity.x, 0, ATTACK_ACCELERATION)
	character_body.velocity.y = move_toward(character_body.velocity.y, 0, ATTACK_ACCELERATION)
	...
	if bodies:
		...
		for body in bodies:
			...
			if body.get_script() == Player: 
				body._get_hit(character_body.attack_damage, character_body.direction_to_player) 
